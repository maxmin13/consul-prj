...
####
STEP "${instance_key} box overlay network provision."
####
...
ssh_run_remote_command_as_root "${remote_dir}"/overlaynet/overlaynet-install.sh \
       ...
       "${ssh_port}" \
       "${user_nm}" \
       "${user_pwd}" >> "${LOGS_DIR}"/"${logfile_nm}" && echo 'Overlay network successully created.' ||
       {
          echo 'ERROR: creating overlay network.'
          exit 1
       } 
