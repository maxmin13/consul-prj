...
then
   ...
   create_internet_gateway "${DTC_INTERNET_GATEWAY_NM}" "${data_center_id}"
   get_internet_gateway_id "${DTC_INTERNET_GATEWAY_NM}"
   internet_gate_id="${__RESULT}"
...
then
   ...
   create_route_table "${DTC_ROUTE_TABLE_NM}" "${data_center_id}"
   get_route_table_id "${DTC_ROUTE_TABLE_NM}"
   route_table_id="${__RESULT}"
...
set_route "${route_table_id}" "${internet_gate_id}" '0.0.0.0/0' > /dev/null
...
then
   ...
   create_subnet "${DTC_SUBNET_MAIN_NM}" \
       "${DTC_SUBNET_MAIN_CIDR}" "${DTC_AZ_1}" "${data_center_id}" "${route_table_id}"
